...
def latent_visual_prompt_inpainting(width=1920, height=1024):
    ...
    for i, img in enumerate([image_first, image_second]):
        ...
        ax.axis("off")
    fig.savefig(f"{CACHE_DIR}/output/14_latent_inpainting.png")

if __name__ == "__main__":
    with stub.run():
        latent_visual_prompt_inpainting.call()
    subprocess.run(
        f'modal nfs get model-cache-vol output/14_latent_inpainting.png output', shell=True)
