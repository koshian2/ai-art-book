...
def merge_and_save():
    pipe_orange = StableDiffusionPipeline.from_pretrained(
        "H:/diffusion_models/diffusers/AOM3A1B_orangemixs", torch_dtype=torch.float16)
    ...
    pipe.vae =  AutoencoderKL.from_pretrained(
        "H:/diffusion_models/diffusers/vae_kl-f8-anime2", torch_dtype=torch.float16)
    pipe.scheduler = UniPCMultistepScheduler.from_config(pipe.scheduler.config)
    pipe.save_pretrained("H:/diffusion_models/diffusers/merge_Counterfeit-V3.0_orangemix")
